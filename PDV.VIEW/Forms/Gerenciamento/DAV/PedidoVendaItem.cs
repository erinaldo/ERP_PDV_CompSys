...
namespace PDV.VIEW.Forms.Gerenciamento.DAV
{
    public partial class PedidoVendaItem : Form
    {
        ...
        public PedidoVendaItem(int idVenda)
        {
            ...
            else
            {
                ...
                if (Venda.IDCliente != null)
                {
                    ...
                    cnpjIntermediadorTextBox.Text = Venda.cnpjintermediador;
                    nomeintermediadorTextBox.Text = Venda.nomeintermediador;
                    indicadorDeIntermediaçãoCheckBox.Checked = Venda.IndiIntermediador;
                    textEditTotalFrete.Text = Venda.totalfrete.ToString();
        ...
        private void salvarSimpleButton_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show(this, "Deseja Salvar o Pedido?", "Pedido de Venda", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                try
                {
                    ...
                    if (indicadorDeIntermediaçãoCheckBox.Checked)
                    {
                        if (string.IsNullOrEmpty(cnpjIntermediadorTextBox.Text))
                        {
                            throw new Exception("Informe o CNPJ do Intermediador");
                        }
                        if (string.IsNullOrEmpty(nomeintermediadorTextBox.Text))
                        {
                            throw new Exception("Informe o nome do intermediador");
                        }
                    }
                    ValidarPagamentos();
                    PreencherVenda();
        ...
        private void PreencherVenda()
        {
            ...
            //Intermediador
            Venda.IndiIntermediador = indicadorDeIntermediaçãoCheckBox.Checked;
            Venda.cnpjintermediador = cnpjIntermediadorTextBox.Text;
            Venda.nomeintermediador = nomeintermediadorTextBox.Text;
            Venda.totalfrete = decimal.Parse(textEditTotalFrete.Text.ToString());
        }
