...
namespace PDV.VIEW.Forms.Consultas
{
    public partial class FCO_Produto : DevExpress.XtraEditors.XtraForm
    {
        ...
        public StringBuilder dataMGV6 { get; set; }

        private void simpleButton6_Click(object sender, EventArgs e)
        {
            dataMGV6 = new StringBuilder();

            try
            {
               
                var produto = FuncoesProduto.GetProdutoLista();
                
                foreach (var item in produto)
                {
                    if (!string.IsNullOrEmpty(item.CodigoBalanca) || item.CodigoBalanca != null)
                    {
                        dataMGV6.Append($"01000{item.CodigoBalanca}{item.ValorVenda.ToString().Replace(".","").Replace(",","").PadRight(6, '0')}{item.Descricao.PadRight(50,' ')}");
                        dataMGV6.AppendLine();
                    }
                }
                string caminhoArquivo = null;
                caminhoArquivo = "C:\\ArquivoBalança";

                if (!Directory.Exists(caminhoArquivo))
                    Directory.CreateDirectory(caminhoArquivo);

                if (!Directory.Exists(Path.Combine(caminhoArquivo, $"{DateTime.Now.ToString("yyyyMMdd")}")))
                    Directory.CreateDirectory(Path.Combine(caminhoArquivo, $"{DateTime.Now.ToString("yyyyMMdd")}"));

                caminhoArquivo = Path.Combine(caminhoArquivo, $"{DateTime.Now.ToString("yyyyMMdd")}");

                caminhoArquivo = Path.Combine(caminhoArquivo, $"Arq{ DateTime.Now.ToString("ddMMyyyyHHmmss") }.txt");


                FileStream fs = new FileStream(caminhoArquivo, FileMode.Append, FileAccess.Write, FileShare.Write);
                fs.Close();

                // using (var file = new StreamWriter(new FileStream(cartaTelegramaPath, FileMode.Open, FileAccess.ReadWrite), Encoding.ASCII))
                using (var file = new StreamWriter(caminhoArquivo, true, Encoding.Default))
                {
                    file.WriteLine(dataMGV6);
                }
