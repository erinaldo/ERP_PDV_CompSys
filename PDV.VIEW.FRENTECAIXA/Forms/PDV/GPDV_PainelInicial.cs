...
namespace PDV.VIEW.FRENTECAIXA.Forms
{
    public partial class GPDV_PainelInicial : Form
    {
        ...
        //    catch (Exception ex)
        //    {
        //        MessageBox.Show(ex.Message, "Comp-Sys EVO", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
        //    }
        //}
        ...
        private void GPDV_PainelInicial_Load(object sender, EventArgs e)
        {
            ...
            //Resizable = false;
            CarregarPainelCategorias();
            Text = string.Format("Comp-Sys  {0} - Suporte {1}", Contexto.VERSAO, "");
        ...
        private void VerificaAberturaCaixa()
        {
            ...
            switch (Caixa.TipoDeVenda)
            {
                ...
                case "SAT":
                    TipoFiscal = 3;
                    lblInfo2.Text = $"CAIXA ONLINE (SAT)";
                    break;
            }
        }
        ...
        public static string FormatarSintegra(string texto, int tamanhoDesejado, bool acrescentarDireita, char caracterAcrescentar)
        {
            string textoRetorno = "";
        ...
        public bool LerEtyiqueta { get; set; }
        private void GetProduto()
        {
            try
            {
                ...
                {
                    if (codigo.Substring(0, 1) == "2")
                    {
                        codigo = codigo.Substring(1, 6);
                        var valor = ovTXT_CodigoBarrasProduto.Text.Trim().Substring(7, 5).ToString();

                        ProdutoPDV = FuncoesProduto.GetProdutoPDVPorCodigoCodigoBalanca(codigo);

                        if (ProdutoPDV == null)
                        {
                            throw new Exception("Produto de etiqueta não foi localizado pelo sistema, verifique o cadastro na tela de produtos.");
                        }

                        ovTXT_Quantidade.Text = "1";
                        ovTXT_ValorUnitario.Text = (Convert.ToDouble(valor) / 100).ToString("n2");
                        ovTXT_SubTotal.Text = (Convert.ToDouble(valor) / 100).ToString("n2");
                        LerEtyiqueta = true;
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"Erro ao ler codigo do produto . Detalhe : {ex.Message}");
            }
        ...
        public bool VerificaEstoque(decimal IDProduto)
        {
            ...
            DataRow DrSaldoAlmoxarifado = FuncoesItemTransferenciaEstoque.GetProdutosComSaldoEmAlmoxarifado(Prod.IDAlmoxarifadoSaida ?? 0, Prod.IDProduto);
        ...
        private void ChamarFinalizarVenda()
        {
            try
            {
                ...
                if (VENDA.ValorTotal >= 1000 && Cliente == null)
                {
                    IdentificarCliente();
                }
