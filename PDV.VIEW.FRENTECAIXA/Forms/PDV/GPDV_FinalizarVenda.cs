...
namespace PDV.VIEW.FRENTECAIXA.Forms
{
        ...
        GPDV_FinalizarVenda : Form
    {
        ...
        public bool retono = false;

        private bool SalvarVendaPDV(bool SAT = false)
        {
            try
            {
                retono = false;
                ...
                if (SAT)
                {
                    var fluxoCaixa = CONTROLER.Funcoes.FuncoesFluxoCaixa.GetFluxoCaixa(TelaInicial.VENDA.IDFluxoCaixa);
                    var caixa = FuncoesCaixa.GetCaixa(fluxoCaixa.CaixaID);
                    EmitirSAT emitirSAT = new EmitirSAT();
                    emitirSAT.TransmitirSAT(TelaInicial, caixa);
            ...
            try
            {
                ...
                if (FormaDePagamento != null)
                {
                    if (FormaDePagamento.Descricao.Contains("CARTAO DE CRÉDITO") || FormaDePagamento.Descricao.Contains("CARTAO DE DÉBITO"))
                        if (Fiscal == 3)
                        {
                            EmitirSAT emitirMFe = new EmitirSAT();
