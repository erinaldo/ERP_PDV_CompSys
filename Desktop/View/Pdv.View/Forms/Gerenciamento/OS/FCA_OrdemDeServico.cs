...
namespace PDV.VIEW.Forms.Gerenciamento.OS
{
    public partial class FCA_OrdemDeServico : Form
    {
        ...
        private void ChecarStatus()
        {
            ...
            {
                if (OrdemDeServico.Status == Status.Faturado)
                {
                    labelStatus.Text = "FATURADO";
                    labelStatus.ForeColor = Color.LightGreen;

                }
                if (OrdemDeServico.Status == Status.Cancelado)
                {
        ...
        {
            gridLookUpEditFormaPagamento.Properties.DataSource = FuncoesFormaDePagamento.GetFormasPagamento();

            gridLookUpEditFormaPagamento.Properties.DataSource = FuncoesFormaDePagamento
                .GetFormasPagamento()
